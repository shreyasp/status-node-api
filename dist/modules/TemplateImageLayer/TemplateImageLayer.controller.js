'use strict';
var __decorate =
  (this && this.__decorate) ||
  function(decorators, target, key, desc) {
    var c = arguments.length,
      r =
        c < 3
          ? target
          : desc === null
          ? (desc = Object.getOwnPropertyDescriptor(target, key))
          : desc,
      d;
    if (typeof Reflect === 'object' && typeof Reflect.decorate === 'function')
      r = Reflect.decorate(decorators, target, key, desc);
    else
      for (var i = decorators.length - 1; i >= 0; i--)
        if ((d = decorators[i]))
          r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
  };
var __metadata =
  (this && this.__metadata) ||
  function(k, v) {
    if (typeof Reflect === 'object' && typeof Reflect.metadata === 'function')
      return Reflect.metadata(k, v);
  };
var __param =
  (this && this.__param) ||
  function(paramIndex, decorator) {
    return function(target, key) {
      decorator(target, key, paramIndex);
    };
  };
Object.defineProperty(exports, '__esModule', { value: true });
const common_1 = require('@nestjs/common');
const TemplateImageLayer_service_1 = require('./TemplateImageLayer.service');
let LayerController = class LayerController {
  constructor(layerService) {
    this.layerService = layerService;
  }
  findLayerForImage(id) {
    return this.layerService.getImageRelatedLayers(id);
  }
  createUpdateTemplateLayer(id, layers) {
    return this.layerService
      .createUpdateTemplateLayers(id, layers)
      .then(() => ({ success: true, message: 'Layer Object created successfully' }))
      .catch(err => err);
  }
};
__decorate(
  [
    common_1.Get(':id'),
    __param(0, common_1.Param('id')),
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [Object]),
    __metadata('design:returntype', void 0),
  ],
  LayerController.prototype,
  'findLayerForImage',
  null,
);
__decorate(
  [
    common_1.Post(':id'),
    __param(0, common_1.Param('id')),
    __param(1, common_1.Body()),
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [Object, Object]),
    __metadata('design:returntype', void 0),
  ],
  LayerController.prototype,
  'createUpdateTemplateLayer',
  null,
);
LayerController = __decorate(
  [
    common_1.Controller('layer'),
    __metadata('design:paramtypes', [TemplateImageLayer_service_1.LayerService]),
  ],
  LayerController,
);
exports.LayerController = LayerController;
//# sourceMappingURL=TemplateImageLayer.controller.js.map
